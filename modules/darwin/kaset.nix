...
let
  ...
  # without the patch.
  #
  # This cannot be inferred, and mas.nix already listing Xcode does not satisfy
  # it: nix builds the whole closure *before* activation, and mas.nix installs
  # *during* it, so a host picking Xcode up for the first time still fails the
  # build that needs it one phase earlier. Xcode also wants
