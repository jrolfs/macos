...
let
  ...
  # trap, so we shell out via import-from-derivation. The probe is named
  # `cache-status-${hash}` after the *current* build's store hash, so it
  # re-runs exactly when that build changes (a nixpkgs bump) — which is when
  # a previously-missing build might have become cached. (Previously bucketed
  # by day via `builtins.currentTime`, but that builtin is unavailable under
  # pure flake evaluation.) Relies on `sandbox = false` (the darwin default)
  # for build-time network; on any HTTP/network failure the probe writes
  # "unknown" rather than failing, so it can't break a rebuild. (The IFD read
  # is deliberately *not* wrapped in tryEval: Nix forbids IFD inside tryEval
  # and raises an uncatchable "path did not exist" error if you try.)
  cacheStatus = drv:
    let
